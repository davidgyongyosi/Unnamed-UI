...
export class IconService {
    ...
    getRenderedContent(icon: IconDefinition | string): Observable<SVGElement> {
        const definition: IconDefinition | null = isIconDefinition(icon) ? (icon as IconDefinition) : this._svgDefinitions.get(icon) || null;
    ...
    protected _loadIconDynamically(type: string): Observable<IconDefinition | null> {
        ...
        if (!inProgress) {
            ...
            const url =
                (namespace ? `${this._assetsUrlRoot}assets/${namespace}/${name}` : `${this._assetsUrlRoot}assets/${icon.theme}/${icon.name}`) +
                suffix;
    ...
    protected _loadSVGFromCacheOrCreateNew(icon: IconDefinition): SVGElement {
        let svg: SVGElement;
        const key =
            icon.theme === 'outline'
        ...
        if (cached) {
            ...
            svg = this._setSVGAttribute(this._createSVGElementFromString(icon.icon));
