...
@Directive({ selector: '[nxIcon]', standalone: false })
export class NxIconDirective implements OnChanges {
    ...
    protected _changeIcon(): Promise<SVGElement | null> {
        return new Promise<SVGElement | null>((resolve) => {
            ...
            this._iconService.getRenderedContent(this._parseIconType(this.type, this.theme)).subscribe((svg) => {
    ...
    protected _getSelfRenderMeta(): RenderMeta {
        return { type: this.type!, theme: this.theme };
    }
